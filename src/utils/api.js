...
export async function fetchUserByEmail(email) {
  ...
  const response = await fetch(`${BASE_URL}/api/users/email/${email}`, options);
  if (!response.ok) {
    throw new Error(response.statusText);
  }
  return await response.json();
...
export async function fetchContacts(userId) {
  const response = await fetch(
    `${BASE_URL}/api/user/contacts?user_id=${userId}`
  );
  if (!response.ok) {
    throw new Error(response.statusText);
  }
...
export async function addToFavorites(userId, contactId) {
  ...
  const response = await fetch(
    `${BASE_URL}/api/user/contacts/favorites`,
    options
  );
  if (!response.ok) {
    throw new Error(response.statusText);
...
export async function deleteFromFavorites(userId, contactId) {
  ...
  const response = await fetch(
    `${BASE_URL}/api/user/contacts/favorites/${contactId}?user_id=${userId}`,
    options
  );
  if (!response.ok) {
    throw new Error(response.statusText);
